...
//#region node_modules/.nitro/vite/services/ssr/assets/auth.signup-CWzCpMNE.js
...
var getInvitationDetails_createServerFn_handler = createServerRpc({
	id: "7ef1ff19c264547fc09a5e6078040e8fcc5f9e4ad5a203fafaee7434ece4388c",
	name: "getInvitationDetails",
	filename: "src/routes/auth.signup.tsx"
}, (opts) => getInvitationDetails.__executeServer(opts));
var getInvitationDetails = createServerFn({ method: "GET" }).validator(objectType({ inviteId: stringType() })).handler(getInvitationDetails_createServerFn_handler, async ({ data }) => {
	const { getDb } = await import("./index.server-ir69dnPx.mjs");
	const { invitations, employees } = await import("./schema-BhmjnuI8.mjs").then((n) => n.f);
	const { eq, and } = await import("../_libs/drizzle-orm+postgres.mjs").then((n) => n.t);
	const db = getDb();
	const [inv] = await db.select({
		email: invitations.email,
		employeeId: invitations.employeeId
	}).from(invitations).where(and(eq(invitations.id, data.inviteId), eq(invitations.status, "pending"))).limit(1);
	if (!inv) return null;
	const [emp] = await db.select({ fullName: employees.fullName }).from(employees).where(eq(employees.id, inv.employeeId)).limit(1);
	return {
		email: inv.email,
...
//#endregion
export { getInvitationDetails_createServerFn_handler, signupAction_createServerFn_handler };
