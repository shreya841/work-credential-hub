...
//#region node_modules/.nitro/vite/services/ssr/assets/performance.functions-CDpE0ahO.js
...
var listReviews = createServerFn({ method: "GET" }).validator(objectType({ employeeId: stringType().uuid("Invalid employee ID") })).handler(listReviews_createServerFn_handler, async ({ data }) => {
	...
	if (user.role !== "super_admin" && user.role !== "company_admin" && user.role !== "hr" && user.id !== employee.userId) throw new Error("You do not have access to this employee's reviews.");
