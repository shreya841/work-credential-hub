...
//#region node_modules/.nitro/vite/services/ssr/assets/employees.functions-nJ05B5tm.js
...
var regenerateEmployeeInviteLink_createServerFn_handler = createServerRpc({
	id: "58fbdaa32d4530bd58aec9f454d5ca9a713ddc4a0c764181306c08f7fddc0859",
	name: "regenerateEmployeeInviteLink",
	filename: "src/lib/api/employees.functions.ts"
}, (opts) => regenerateEmployeeInviteLink.__executeServer(opts));
var regenerateEmployeeInviteLink = createServerFn({ method: "POST" }).validator(objectType({ employeeId: stringType().uuid("Invalid employee ID") })).handler(regenerateEmployeeInviteLink_createServerFn_handler, async ({ data }) => {
	const user = await requireAuth();
	await requireVerifiedCompany(user);
	const db = getDb();
	const [employee] = await db.select().from(employees).where(eq(employees.id, data.employeeId)).limit(1);
	if (!employee) throw new Error("Employee not found");
	if (employee.claimStatus !== "unclaimed") throw new Error("Employee profile has already been claimed");
	if (user.role !== "super_admin" && employee.companyId !== user.companyId) throw new Error("Unauthorized to access this employee's invite link");
	const [comp] = await db.select({ name: companies.name }).from(companies).where(eq(companies.id, employee.companyId)).limit(1);
	const companyName = comp?.name || "your employer";
	await db.delete(invitations).where(eq(invitations.employeeId, employee.id));
	const [newInv] = await db.insert(invitations).values({
		email: employee.email.toLowerCase().trim(),
		companyId: employee.companyId,
		employeeId: employee.id,
