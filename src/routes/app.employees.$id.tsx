...
export const Route = createFileRoute("/app/employees/$id")({
  ...
  notFoundComponent: () => <div className="p-8">Employee not found</div>,
});
...
function EmployeeDetail() {
  ...
  const history = myReviews.slice().reverse().map((r) => ({ d: r.date.slice(0, 7), rating: r.overall }));

  return (
    <div>
      <Button asChild variant="ghost" size="sm" className="mb-4 -ml-2"><Link to="/app/employees"><ArrowLeft className="mr-1 h-4 w-4" /> Back to employees</Link></Button>
      ...
      <Card className="overflow-hidden border-border/60 bg-gradient-card">
        ...
        <CardContent className="-mt-12 p-6">
          <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] gap-4">
            <Avatar className="h-24 w-24 shrink-0 border-4 border-background shadow-elegant"><AvatarImage src={e.photo} /><AvatarFallback>{e.fullName[0]}</AvatarFallback></Avatar>
            <div className="min-w-0 self-end">
              ...
              <p className="text-sm text-muted-foreground">{e.designation} · {e.department} · {company?.name}</p>
            </div>
            <div className="self-end">
              <Button asChild variant="outline" size="sm"><Link to="/profile/$id" params={{ id: e.id }} target="_blank">Public profile</Link></Button>
            </div>
          </div>
          <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <Info icon={Calendar} label="Joined" value={e.joiningDate} />
          </div>
        </CardContent>
      </Card>
      ...
      <Tabs defaultValue="performance" className="mt-6">
        ...
        <TabsContent value="performance" className="mt-4 space-y-5">
          <div className="grid gap-5 lg:grid-cols-2">
            <Card className="border-border/60"><CardHeader><CardTitle>Latest rating breakdown</CardTitle></CardHeader>
            ...
            <Card className="border-border/60"><CardHeader><CardTitle>Overall rating history</CardTitle></CardHeader>
              <CardContent className="h-72">
                <ResponsiveContainer width="100%" height="100%">
            ...
            {radarData.map((m) => (
              <Card key={m.metric} className="border-border/60 bg-gradient-card">
                <CardContent className="p-4">
                  ...
                  <div className="mt-1 font-display text-2xl font-bold">{m.v.toFixed(1)} <span className="text-sm text-muted-foreground">/5</span></div>
                  <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-muted"><div className="h-full bg-gradient-hero" style={{ width: `${(m.v / 5) * 100}%` }} /></div>
                </CardContent>
              </Card>
            ))}
          </div>
          ...
          <Card className="border-border/60"><CardHeader><CardTitle>Achievements</CardTitle></CardHeader>
            <CardContent className="space-y-3">
              {["Top performer Q3 2024", "Led platform migration", "Mentored 5 engineers"].map((a) => (
                <div key={a} className="flex items-center gap-2 text-sm"><Award className="h-4 w-4 text-warning" /> {a}</div>
              ))}
            </CardContent>
          </Card>
        </TabsContent>
...
function Info({ icon: Icon, label, value }: { icon: any; label: string; value: string }) {
  return (
    <div className="flex items-center gap-3 rounded-lg border border-border/60 bg-background/40 p-3">
      ...
      <div className="min-w-0"><div className="text-[10px] uppercase tracking-wider text-muted-foreground">{label}</div><div className="truncate text-sm font-medium">{value}</div></div>
    </div>
  );
}
