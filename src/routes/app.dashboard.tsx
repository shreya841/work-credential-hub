      ...
      if (user.role === "super_admin") {
        ...
        if (companyId) {
          const company = await queryClient.ensureQueryData({
            ...
            queryFn: () => getCompanyById({ data: { id: companyId } }),
          });
          companyStatus = company?.status;
        }
        if (companyStatus === "approved" && user.companyId) {
          await Promise.all([
            ...
            queryClient.prefetchQuery({
              queryKey: ["dashboard-department-analytics"],
              queryFn: () => getDepartmentAnalytics(),
            }),
            queryClient.prefetchQuery({
              queryKey: ["dashboard-verification-statistics"],
...
function HrDashboard() {
  ...
  const { data: departmentAnalytics, isLoading: departmentLoading } = useQuery({
    queryKey: ["dashboard-department-analytics"],
    queryFn: () => getDepartmentAnalytics(),
    enabled: isApproved,
  });

  const { data: verificationStats, isLoading: verificationStatsLoading } = useQuery({
    queryKey: ["dashboard-verification-statistics"],
    queryFn: () => getVerificationStatistics(),
    enabled: isApproved,
  });
